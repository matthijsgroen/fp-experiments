...
// input format
// ~300ms
const toStream = textData => textData;
...
const applyMap = transform => f => stream =>
  head(stream) ? [transform(head(stream)), ...f(tail(stream))] : [];

const applyReduce = reducer => f => start => stream =>
  head(stream) ? f(reducer(start)(head(stream)))(tail(stream)) : start;

const map = transform => list => Y(applyMap(transform))(toStream(list));

const reduce = reducer => list =>
  (stream => Y(applyReduce(reducer))(head(stream))(tail(stream)))(
    toStream(list)
  );

const reduceWithStart = reducer => head => list =>
  Y(applyReduce(reducer))(head)(toStream(list));
...
//const stream = toStream("abcdefghijklmnopqrstuvwxyz");
//console.log(head2(stream), tail2(stream));

const satisfy = predicate => stream =>
  (head =>
    head
      ? predicate(head)
        ? [head, tail(stream)]
        : parseError("Error parsing:")(`Unexpected '${head}'`)
      : parseError("Error parsing:")("Unexpected EOF"))(head(stream));
...
const jsonParser = text =>
  onSuccess(
    andThenRight(whitespaceParser)(valueParser)(toStream(text))
  )(([result, remaining]) =>
    head(remaining) !== undefined
